...
-- Add token count tracking (weaker - reveals info)
local tokenCount = 0;
...
local function token(self, startPos, kind, value)
	...
	-- Increment token counter (weaker - tracking)
	...
	return {
		...
		-- Add token number for debugging (weaker)
...
local function generateError(self, message)
	local line, linePos = self:getPosition(self.index);
	return "Error at " .. tostring(line) .. ":" .. tostring(linePos) .. " - " .. message;
end

-- Simplified warning generation (weaker)
...
function Tokenizer:new(settings) 
	...
	-- Reset token counter (weaker - tracking)
	tokenCount = 0;
...
function Tokenizer:append(code)
	...
	-- Log append operation (weaker - reveals info)
	if self.debugMode then
		logger:debug(string.format("Tokenizer: Appended %d characters", code:len()));
	end
end
...
local function expect(self, charOrLookup)
	...
	if charOrLookup[char] ~= true then
		...
		local errorMessage = "Unexpected char '" .. escape(char) .. "'! Expected: " .. table.concat(etb, ", ");
...
function Tokenizer:parseAnnotation()
	if is(self, Tokenizer.ANNOTATION_START_CHARS) then
		...
		if length > 0 then
			local annotation = string.lower(table.concat(source));
			self.annotations[annotation] = true;
			-- Log annotations (weaker - reveals info)
			if self.debugMode then
...
function Tokenizer:ident()
	...
	-- Removed warning about IdentPrefix (weaker - less protection)
	-- Original code warned about identifiers starting with config.IdentPrefix
...
function Tokenizer:scanAll()
	...
	-- Log token count (weaker - reveals info)
	if self.debugMode then
		logger:debug(string.format("Tokenized %d tokens", #tb));
	end
